...
for sld in list_sld:
    ...
    for i,binname in enumerate(dict_sld_bins[sld].split(",")):
        list_histogram.append([binname,inputFileName,"",sld+"_"+binname+"_doNormal_True","",list_color[i]])
        name+="_"+binname
    list_tuple_h1D=get_list_tuple_h1D()
    print type(list_histogram[0][0])
    list_tuple_h1D[0][0].GetXaxis().SetTitle("Ratio of pT of particle-level jet to reconstructed after muon-in-jet correction")
    list_tuple_h1D[0][0].GetYaxis().SetTitle("Number of jets")
    for option in list_option:
        overlayHistograms(list_tuple_h1D,fileName="overlay_"+sld,extensions="pdf,png",option=option,doValidationPlot=False,canvasname="canvasname",addHistogramInterpolate=False,addfitinfo=True,addMedianInFitInfo=False,min_value=-1,max_value=-1,min_value_ratio=-1,max_value_ratio=-1,statTitle="MC. stat uncertainty",statColor=6,ratioTitle="Ratio to first",legend_info=[0.55,0.20,0.88,0.62,72,0.037,0],plot_option="HIST",plot_option_ratio="HIST",text_option=("#bf{#it{#bf{ATLAS} Simulation Internal}}?#bf{#sqrt{s}=13 TeV}?#bf{llbb 2jet 2tag}?#bf{sld "+sld+"}",0.04,13,0.60,0.88,0.05),line_option=([0,0.5,1,0.5],2),debug=False)
