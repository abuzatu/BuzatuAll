...
#scalesstring="OneMu,PtRecollbbOneMuTruthWZNoneOld,PtRecollbbOneMuPartonBukinNew,PtRecollbbOneMuPartonBukinNewTrue,PtRecollbbOneMuTruthWZNoneNewTrue,PtRecollbbOneMuTruthWZBukinMedianNewTrue,Regression"
scalesstring="PtRecollbbOneMuPartonBukinNewTrue,PtRecollbbOneMuTruthWZNoneNewTrue,PtRecollbbOneMuTruthWZBukinMedianNewTrue"
...
for process in list_processes:
    ...
    for i,variation in enumerate(list_variation):
        list_histogram.append([variation,"/Users/abuzatu/data/histos_mbb_fromReader/submitDir_"+variation+"/hist-ZHll125.root","","qqZllH125_2tag2pjet_0_500ptv_SR_mBB","",list_color[i]])    
    list_tuple_h1D=get_list_tuple_h1D()
    overlayHistograms(list_tuple_h1D,fileName="overlay_comparing_variation_"+process,extensions="pdf",option="histo",doValidationPlot=False,canvasname="canvasname",addHistogramInterpolate=False,addfitinfo=True,addMedianInFitInfo=False,min_value=-1,max_value=-1,min_value_ratio=-1,max_value_ratio=-1,statTitle="MC. stat uncertainty",statColor=6,ratioTitle="Ratio to first",legend_info=[0.15,0.30,0.48,0.72,72,0.030,0],plot_option="HIST",plot_option_ratio="HIST",text_option=("#bf{#it{#bf{ATLAS} Simulation Internal}}?#bf{#sqrt{s}=13 TeV}?#bf{"+process+"}?#bf{"+variation+"}",0.04,13,0.37,0.88,0.03),line_option=([0,0.5,1,0.5],2),debug=debug)
