...
if total!=2:
    ...
    print "Ex: ",sys.argv[0]," mbb_1_20GeV"
    assert(False)
# done if
...
#list_color=[7,1,2,4,3]
list_color=[2,4,1]
list_correction="Nominal,OneMu,PtRecoTruthWZNone,PtRecoFromTTbar,PtRecoTruthWZGauss".split(",")
list_correction="PtRecoTruthWZNone,PtRecoFromTTbar,PtRecoTruthWZGauss".split(",")
#list_correction="Nominal,OneMu,PtRecoTruthWZNone,PtRecoTruthWZBukinMedian,PtRecoFromTTbar".split(",")
#list_correction="TruthWZ,OneMu,PtRecoTruthWZNone,PtRecoTruthWZBukinMedian,PtRecoFromTTbar".split(",")
...
"PtRecoTruthWZNone":"after PtReco (Hbb)",
"PtRecoTruthWZGauss":"after PtReco (HbbG Gauss)",
"PtRecoTruthWZBukinMedian":"after PtReco (Bukin median)",
"PtRecoFromTTbar":"after PtReco (ttbar)",
...
if True:
    ...
    for i,correction in enumerate(list_correction):
        list_histogram.append([dict_correction_correctionLegend[correction],"/Users/abuzatu/data/histos_mbb/161001/"+suffix+".root","","h_"+correction+"_Mass","",list_color[i]])
        name+="_"+correction
    list_tuple_h1D=get_list_tuple_h1D()
    print type(list_histogram[0][0])
    for option in list_option:
        overlayHistograms(list_tuple_h1D,fileName="161001/overlay"+"_"+suffix,extensions="pdf",option=option,doValidationPlot=False,canvasname="canvasname",addHistogramInterpolate=False,addfitinfo=True,addMedianInFitInfo=False,significantDigits=("3","1","1","3"),min_value=-1,max_value=-1,doRatioPad=False,min_value_ratio=-1,max_value_ratio=-1,statTitle="MC. stat uncertainty",statColor=6,ratioTitle="Ratio to first",legend_info=[0.55,0.34,0.88,0.65,72,0.037,0],plot_option="HIST",plot_option_ratio="HIST",text_option=("#bf{#it{#bf{ATLAS} Simulation Internal}}?#bf{#sqrt{s}=13 TeV}?#bf{b-tagged jet}",0.04,13,0.60,0.88,0.05),line_option=([0,0.5,1,0.5],2),debug=False)
