...
def get_dict_sld_histo_TTbar(debug):
    fileName="/Users/abuzatu/data/histos_PtReco/ttbar/Topo_AllMu_corr_truthWZ.root"
    dict_sld_histo={}
    ...
    return dict_sld_histo
# done function

def get_dict_sld_histo_Hbb_Gauss(debug):
    fileName="/Users/abuzatu/data/histos_PtReco/PtRecoNow/PtReco_histos_llbb_OneMu_TruthWZ_True.root" 
    dict_sld_histo={}
    dict_sld_histo["nosld"]=retrieveHistogram(fileName=fileName,histoPath="",histoName="PtReco_hadronic_Gauss",name="",debug=debug)
    dict_sld_histo["mu"]=retrieveHistogram(fileName=fileName,histoPath="",histoName="PtReco_semileptonic_Gauss",name="",debug=debug)
    dict_sld_histo["el"]=retrieveHistogram(fileName=fileName,histoPath="",histoName="PtReco_semileptonic_Gauss",name="",debug=debug)
    return dict_sld_histo
# done function
...
def createHistogram():
    ...
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_1_20GeV.root"
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_2_20GeV_TTbar275.root"
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_3_25GeV_TTbar275.root"
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_4_30GeV_TTbar275.root"
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_5_65GeVCutOnMbbFromTTbarPtReco.root"
    #outputFileName="/Users/abuzatu/data/histos_mbb/160930/mbb_7_20GeV_2sigma.root"
    outputFileName="/Users/abuzatu/data/histos_mbb/160901/mbb_1.root"
    ...
    scalesstring="TruthWZ,Nominal,OneMu,PtRecoTruthWZNone,PtRecoTruthWZBukinMedian,PtRecoTruthWZGauss,PtRecoFromTTbar,Regression"
    ...
    for i, event in enumerate(inputTree):
        ...
        if debug:
            print "******* new event **********"
        #
        Pt1 = getattr(event,"b1_Nominal_Pt")
        Pt2 = getattr(event,"b2_Nominal_Pt")
        # remove subleading jet with smaller than 25 GeV
        if Pt2<20.0:
            continue
        ...
        for scale in scales:
            ...
            for b in bs:
                ...
                if scale=="PtRecoFromTTbar" or scale=="PtRecoTruthWZGauss":
                    ...
                    if debug:
                        ...
                        print "sldname",sldname
                    if scale=="PtRecoFromTTbar":
                        dict_sld_histo=get_dict_sld_histo_TTbar(debug)
                    elif scale=="PtRecoTruthWZGauss":
                        dict_sld_histo=get_dict_sld_histo_Hbb_Gauss(debug)
                    else:
                        print "scale",scale,"not known. Choose PtRecoFromTTbar, PtRecoTruthWZGauss. Will ABORT!!!"
                        assert(False)
                    # done if
                    if debug:
                        "type of hist",type(dict_sld_histo[sldname])
        ...
        # reject the event if the mbb from PtReco from TTbar falls below 65 GeV
        # as we noticed better resolution in that case with respect to the regular corrections
        # but we want to compare all corrections for teh same event
        #if dict_scale_obj_tlv["PtRecoFromTTbar"]["bb"].M() < 65.0: 
        #    continue

        # now start loop over scales again
        for scale in scales:
